    ...
    <>
      ...
      <section className="hero">
        <div className="hero-inner">
          ...
          <div className="hero-buttons">
            ...
            <Link href="/recherche" className="btn-hero-ghost">
              Voir les annonces
            </Link>
          </div>
      ...
      <section className="wrap" style={{ paddingTop: '4rem' }}>
        ...
        <div className="grid-2">
          <UserCard
            ...
            tags={['Statistiques', 'Highlights', 'Historique', 'Candidature']}
            tagClass="badge-blue"
          />
          <UserCard
            href="/recherche"
      ...
      <section style={{ background: '#fff', marginTop: '3rem' }}>
        <div className="wrap">
          ...
          <p className="section-desc">
            Les clubs et coachs publient leurs besoins en temps réel. Postule directement depuis la
            plateforme.
          </p>
          <div className="grid-2">
            {annonces.map((a) => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 14 }}>{a.authorName}</div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{a.createdAt}</div>
                  </div>
                </div>
                <p style={{ fontSize: 15, lineHeight: 1.5, marginBottom: 12 }}>{a.body}</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
                  <span className="badge badge-blue">{a.ligue}</span>
                  {a.position && <span className="badge badge-amber">{a.position}</span>}
                  <span className="badge badge-green">📍 {a.zone}</span>
                </div>
...
function UserCard({
  ...
  const stripeColor =
    color === 'blue'
      ...
      : 'linear-gradient(90deg,#1db954,#0d7a36)';
  ...
  return (
    ...
    >
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: 14, lineHeight: 1.6, marginBottom: '1.25rem' }}>
        {desc}
      </p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        {tags.map((t) => (
          <span key={t} className={`badge ${tagClass}`}>
            {t}
          </span>
        ))}
      </div>
